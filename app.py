...
# 1. 获取当前时间
now = datetime.now()
# 2. 定义一个列表来把数字星期转换成中文（Python的weekday() 0=周一，所以需要调整）
weekdays = ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"]
weekday_name = weekdays[now.weekday()]
# 4. 拼接成最终格式：2026-05-08 星期五
...
def process_data(df, city, status, volume_col='体积m3'):
    """
    处理上传的数据框。
    注意：为了简化网页版，我们去掉了原脚本中复杂的文件名自动匹配逻辑，
    改为由网页按钮直接传参。
    """
    # 1. 筛选城市
...
FINAL_COLUMNS = [
    ...
    "以星海卡（带车架）", "以星海卡（不带车架）", 
    "普船海派", "普船海卡", 
    "ZIM-CA", "ZIM-CA16", "ZIM-KY19", "ZIM-KY23", "ZIM-KY5",
    "普船-CA1", "普船-CA2", "普船-CA5", 
    "普船-KY", "普船-特惠KY3", "普船-特惠KY5", 
    "普船-NYC拆送", "普船-GA直送", "普船-TX直送"
]
...
if st.sidebar.button("🚀 开始处理数据", type="primary"):
    if not all([uploaded_order, uploaded_in, uploaded_load]):
        ...
        with st.spinner('处理中...请稍候...'):
            ...
            row_data = {col: 0 for col in FINAL_COLUMNS}
            first_column_name = FINAL_COLUMNS[0] 
            row_data[first_column_name] = HEADER_TEXT
